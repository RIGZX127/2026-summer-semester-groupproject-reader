...
- AI 标签入口已从文章右键菜单移入“管理标签”窗口，并支持原地生成、错误提示与建议标签选择。
- Reader 翻译已移除冲突内联色并使用主题语义色；按钮尺寸与浅色 AI 设置文字对比度已统一。
- 标签管理滚动区已显式继承主题背景；工具栏按钮保留 36 像素热区并将可见边框内缩为约 32 像素。
- 所有纯图标按钮静止态统一透明无边框，仅在悬停、焦点、按下或选中时显示反馈。
- 订阅源、收藏夹与文章选择支持再次点击取消；订阅源和收藏夹选择保持互斥。
- 通用设置新增 0–300 秒的文章停留已读延迟，切换文章会取消旧计时。
- 当前完整回归为 277 项测试通过。
